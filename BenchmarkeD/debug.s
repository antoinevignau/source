*
* Time to read a file in 8-bits ** DEBUG ONLY **
*
...
REF_1MHZ	=	1023000
DEF_RRC	=	6	; read in a 32K buffer by default
...
*--- Display buffer information
	...
	lda	#2	; $0202 : 514 iterations
	sta	maxINDEX
	lda	#2
	sta	maxINDEX+1	; DEBUG
...
]lp	jsr	displayWHEEL	; show a spinning /
	jsr	checkCANCEL	; press ESC to quit
	bcs	readFILE4	; exit if so

	jsr	PRODOS	; read xK
	dfb	$ca
	da	proREAD
	
	jsr	DEBUG	; display all info
	bcs	readFILEERR	; read error?

	inc	theINDEX	; next counter
	bne	readFILE1A
	inc	theINDEX+1
readFILE1A
	lda	theINDEX+1	; the max counter
	cmp	maxINDEX+1
	bcc	]lp
	lda	theINDEX
	cmp	maxINDEX
	blt	]lp
	jmp	readFILE2	; exit if reached
	
readFILEERR	cmp	#$4c	; we had an error, check which one
	beq	readFILE2	; if EOF, exit and display end date
	
	cmp	#$27	; I/O error
	bne	readFILE2	; if not, exit and display end date
...
DEBUG_A	lda	#$00
	...
	jsr	PRBYTE
	lda	theINDEX
	jsr	PRBYTE	; current counter
	lda	#"-"
	jsr	COUT
	lda	proREAD+5
	jsr	PRBYTE
	lda	proREAD+4
	jsr	PRBYTE	; request count
	lda	#"-"
	jsr	COUT
	lda	proREAD+5
	jsr	PRBYTE
	lda	proREAD+4
	jsr	PRBYTE	; transfer count
	lda	#$8d
	jsr	COUT	; CR
	plp		; restore status and exit
	rts
...
theRRC	dw	DEF_RRC	; 0..6 (default is 512 bytes)
...
proDESTROY	dfb	$1
	da	pathname1	; +1 pathname

proCREATE	dfb	$7
	da	pathname1	;  +1 pathname
	dfb	$e3	;  +3 access
	dfb	$06	;  +4 file_type (bin)
	dw	$bdbd	;  +5 aux_type
	ds	1	;  +7 storage_type
	ds	2	;  +8 create_date
	ds	2	; +10 create_time
...
proWRITE
	dfb	$4
	ds	1	; +1 ref_num
	da	ptrBUFFER	; +2 data_buffer
	ds	2	; +4 request_count ** variable **
	ds	2	; +6 transfer_count
